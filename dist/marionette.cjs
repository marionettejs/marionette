...
assignOwn(CollectionView.prototype, ViewMixin, {
  ...
  _onCollectionUpdate(collection, options) {
    ...
    const canRemoveWithoutRender = this._isRendered && changes.removed.length > 0 && changes.added.length === 0 && changes.merged.length === 0 && isDefaultComparator && isDefaultFilterQuery && isDefaultSort && isDefaultFilter && isRemovalOrderStable && !this.viewFilter && this.children.length === this._children.length && this._children.length > 0 && !this._hasUnrenderedViews && !this._emptyRegion.hasView();

...
test('documented form preserves keyboard, validation, draft, focus, retry and teardown behavior', async({ umdPage: page, browserName: name }) => {
  await page.evaluate(() => { document.querySelector('#content').id = 'form'; });
  await page.addScriptTag({ content: `${browserCode}\nglobalThis.ProfileForm = ProfileForm;` });
  await page.evaluate(() => {
    globalThis.saves = [];
    globalThis.region = new globalThis.Marionette.Region({ el: document.querySelector('#form') });

...
const feature = await mountFeature(document.querySelector('#feature'), 'first-feature');
assert.equal(feature.application.isRunning(), true);
assert.equal(document.querySelector('#feature').textContent, 'Ready');
const other = await mountFeature(document.querySelector('#app'), 'other-feature');
const otherView = other.application.getView();
const previous = feature.application.getView();
feature.refresh();
assert.equal(previous.isDestroyed(), true);
assert.equal(other.application.getView(), otherView);

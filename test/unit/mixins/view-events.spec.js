...
describe('view events mixin', function() {
  it('resolves callable event and trigger maps on the view with no arguments', function() {
    ...
    const view = createView({ events, triggers });
    const delegates = [];
  ...
  it('treats nullish resolved maps as no-ops', function() {
    const view = createView({
      events() { return null; },
      triggers() { return undefined; }
    });
    const delegates = [];

...
test('render resource survives creation and releases once on replacement and destruction', t => {
  ...
  const view = solution.createHost(acquire);
  ...
  assert.equal(records[1].disposals, 1);
  assert.equal(records[2].disposals, 0);
  assert.ok(records[2].view === view, 'Acquisition must receive the owning View');

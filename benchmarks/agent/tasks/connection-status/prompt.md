...
Implement `solution.mjs` in this workspace. Return an already-rendered View whose Behavior subscribes once to `statusSource.subscribe(callback)`, which returns an unsubscribe function. The callback may deliver its initial text synchronously during subscription. Show its text in an output element; replace any preexisting host markup with the View template. Rendering again must preserve one subscription and latest status. Destroy unsubscribes exactly once. This is a View concern, not an Application.

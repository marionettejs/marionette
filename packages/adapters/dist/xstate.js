...
function createXStateActorApi({
  ...
  const ActorApi = {
    ...
    subscribe(actor, eventName, callback, context) {
      const disposer = snapshotEvent != null && eventName === snapshotEvent ? subscribeSnapshots(actor, snapshot => callback.call(context, snapshot)) : actor.on(eventName, event => callback.call(context, event));
...
export { createXStateActorApi as default };

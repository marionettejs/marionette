...
| Application | Named children belong to the Application until removal. setView temporarily owns a prepared View; showView hands it to the Region and records the selected displayed root. getView never adopts a direct Region display; select one explicitly with setView. | [Application Lifecycle](./marionette.application.md) |

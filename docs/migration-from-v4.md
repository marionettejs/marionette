...
| `getUI` binding lifecycle | Calling `getUI()` while UI elements were unbound failed with an incidental `TypeError`. | View, CollectionView, and Behavior throw `MN0023` when `getUI()` is called before binding or after unbinding. | Changed | Render a templated View or call `bindUIElements()` explicitly before `getUI()`; bind again before calling it after unbinding. |

...
| String handler validation | Some missing string handler references were silently omitted during delegation or binding. | Every supplied string handler must resolve to a callable method; otherwise Marionette throws `MN0019` before delegation or binding. | Changed | Define or remove the named method, or supply the handler as a function. |

...
| Reentrant destruction | Calling `destroy()` from `before:destroy` could recurse or repeat teardown for MnObject, Application, and Region. | Reentrant and later repeated `destroy()` calls return the same instance without restarting teardown. | Changed | Keep `before:destroy` non-throwing. Reentrant calls are ignored, but if `before:destroy` throws, later `destroy()` calls do not retry teardown. |

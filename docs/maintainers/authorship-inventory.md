...
This is the finite path reconciliation for [#329](https://github.com/marionettejs/marionette/issues/329), dated 2026-09-09. It accounts for all **46 historical paths** in the fork `662996b2` to `14db5c1a` inventory and all **62 current executable authored TypeScript files** at production source revision `6bc139c7`. Generated distributions, the generated version module, and `src/version.d.ts` are not executable authored production files. The stabilization and closeout changes add no production framework source.
...
| `packages/adapters/src/data/internal/keyed-snapshot.ts` | Added after the original inventory | Retain ordered keyed snapshot validation shared by optional adapters; no core import. |
| `packages/adapters/src/data/xstate.ts` | Added after the original inventory | Retain explicit actor snapshots/subscriptions and owned-actor disposal. |
...
| `packages/adapters/src/dom/lit-html.ts` | Added after the original inventory | Retain an opt-in DOM adapter with synchronous commit, fixed-root ownership and conformance tests. |
| `packages/adapters/src/dom/morphdom.ts` | Added after the original inventory | Retain an opt-in DOM adapter with synchronous commit, fixed-root ownership and conformance tests. |
| `packages/data/src/api.ts` | Added after the original inventory | Retain native package integration through public DataApi/StateApi protocols. |
| `packages/data/src/collection.ts` | Added after the original inventory | Retain native ordered collection/index ownership; #465 accounts for instance/ID/cid precedence and bounded bulk removal. |
| `packages/data/src/index.ts` | Added after the original inventory | Retain explicit public package exports; isolates optional dependencies and produces declarations. |
| `packages/data/src/model.ts` | Added after the original inventory | Retain native model events and identity; #465 preserves native subclasses. |
| `packages/radio/src/debug.ts` | `modules/common/radio.js` | Retain the internal shared debug state to avoid a real dependency cycle. |
| `packages/radio/src/index.ts` | Added after the original inventory | Retain explicit public package exports; isolates optional dependencies and produces declarations. |
| `packages/radio/src/radio.ts` | `modules/radio.js` | Retain the separately published singleton/channel API; #338 removed unevidenced advanced seams. |
| `packages/radio/src/requests.ts` | `mixins/requests.js` | Retain explicit request/reply ownership and cleanup; no implicit core Radio configuration. |
...
| `packages/utils/src/call-handler.ts` | Added after the original inventory | Retain the directly implemented shared helper and its documented input shape; removes Underscore and duplicate core implementations. |
...
| `packages/utils/src/get-option.ts` | Added after the original inventory | Retain the directly implemented shared helper and its documented input shape; removes Underscore and duplicate core implementations. |
...
| `packages/utils/src/index.ts` | Added after the original inventory | Retain explicit public package exports; isolates optional dependencies and produces declarations. |
...
| `packages/utils/src/set-property.ts` | Added after the original inventory | Retain the directly implemented shared helper and its documented input shape; removes Underscore and duplicate core implementations. |
...
| `src/create-marionette.ts` | Added after the original inventory | Retain isolated class/configuration construction; prevents one application changing another runtime. |
...
| `src/mixins/state.ts` | Added after the original inventory | Retain explicit borrowed/owned state sources and disposal; no provider registry. |
...
| `src/modules/common/chainable-methods.ts` | Added after the original inventory | Retain shared fluent contract types; no per-instance resources. |
...
| `src/modules/common/view.ts` | Added after the original inventory | Retain common View/CollectionView ownership contracts and types. |
...
| `src/runtime-id.ts` | Added after the original inventory | Retain private runtime identity for ownership checks; it is not an adapter or global registry. |
| `src/runtime/data-api.ts` | Added after the original inventory | Retain neutral static model/array reads and the explicit DataApi protocol. |
...
| `src/runtime/state-api.ts` | Added after the original inventory | Retain state subscription/disposal protocol independent of domain data. |
| `src/utils/extend.ts` | Added after the original inventory | Retain the runtime-aware extension boundary; standalone extension remains in utils. |
| `src/utils/subscribe-bindings.ts` | Added after the original inventory | Retain one subscription/release path for documented binding maps. |
...
The old comments describe an intermediate v5 tree. Radio is now `@mnjs/radio`; its implicit core singleton/configuration path is gone. The Backbone prototype shim and automatic Backbone.View lifecycle adaptation are gone; `@mnjs/adapters/backbone` now supplies data/state protocols. View roots stay fixed, so Behavior `_syncElement` and View `setElement` are not retained. The feature registry and compatibility aliases are removed. Stable behavioral diagnostics remain, while TypeScript expresses argument shapes. Historical references to failure-atomic registration do not authorize synchronous rollback: the [current failure boundary](../view.lifecycle.md#synchronous-failures) governs.

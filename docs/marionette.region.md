...
An `empty` listener may show a prepared replacement through the same Region;
that new operation observes the completed empty state. Exceptions still abort
the synchronous callback operation under the [synchronous failure boundary](./view.lifecycle.md#synchronous-failures).

...
disabled; prerelease publication is also disabled (`null`) while RC.2 is prepared. Schema 2 separates `publication.stable`
...
The candidate version is `5.0.0-rc.2`; earlier published versions belong to another
...
Use matching `5.0.0-rc.2` versions across all five packages and their internal
requirements. `publication.stable` remains `false`; `publication.prerelease`
remains `null`. RC.2 preparation does not authorize publication: verify npm
...
and remains a GitHub prerelease. Preparing or validating RC.2 does not change
registry tags. A separately authorized publication would move `latest` to RC.2
for all five packages and leave `next` untouched. Once stable v5 ships, change
...
configuration or a later candidate's result. Existing tarballs are immutable and are not
republished by these checks. Each separately authorized publication must establish the
